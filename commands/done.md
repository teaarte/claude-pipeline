...
5. Clean `.claude/`: first call `pipeline_unlock_writes({project_dir, ttl_seconds:300, reason:"/done cleanup"})` so the guard hook permits direct deletion of MCP-managed files, then delete every orchestrator working file (plan.md, pipeline-state.json, pipeline-state-summary.md, findings.jsonl, driver-state.json, context-doc.md, dependency-audit.md, research-report.md, architecture-decisions.md, migration-plan.md, plan-*.md, implementation-notes*.md, diff.txt, caller-context.md, antipattern-candidates.md, past-misses-*.md, mcp-audit.jsonl, refs-to-load.md, test-files-must-stay-green.json, reviews/, abandoned-*.json, .mcp-managed, .mcp-bypass-allowed); finally call `pipeline_relock_writes({project_dir})` which also unlinks the bypass marker (Q12/Q13 — `relock` is the only correct way to leave the dir clean). Keep `commands/` and `settings.local.json`. Delete root `PLANNING.md` if present.
