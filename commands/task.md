...
### 1a. MANDATORY — Initialize pipeline state (via MCP)

**BEFORE doing anything else in this step**, call the MCP server to initialize pipeline state:

```
mcp__claude-pipeline__pipeline_init({
  project_dir: "<absolute path to project root, i.e. current cwd>",
  task_short: "<5-10 word task description>",
  // task_id is generated by the server as t-<YYYY-MM-DD>-<short-slug>
})
```

This single call:
- Copies `~/.claude/templates/pipeline-state.json` → `.claude/pipeline-state.json` (validated against schema on every subsequent mutation)
- Copies `~/.claude/templates/pipeline-state-summary.md` → `.claude/pipeline-state-summary.md` (rebuilt mechanically after every MCP mutation; never hand-edit)
- Creates empty `.claude/findings.jsonl` (append-only, MCP-managed)
- Sets `task_id`, `started_at`, and `project_dir`

This is a HARD REQUIREMENT — no exceptions, no deferring. Without `pipeline_init`, `/done` cannot collect metrics, structured findings cannot be linked to a task, and cross-session recovery via `/task-continue` is broken.

**Never `Write` or `Edit` `.claude/pipeline-state.json`, `.claude/pipeline-state-summary.md`, or `.claude/findings.jsonl` directly.** All state mutations go through `mcp__claude-pipeline__*` tools. If the MCP server is unavailable (`claude mcp list` does not show `claude-pipeline: ✓ Connected`) — STOP and ask the user to fix the server. Do not fall back to manual file writes.

If `pipeline_init` refuses because a finished task is already present in `.claude/` — surface the error to the user; they must `/done` or clean up before starting a new task.
...
Record `project_stack` and `tests_mode` via `mcp__claude-pipeline__pipeline_set_phase_status` (the MCP `pipeline_init` call accepts these or they can be set on the first `pipeline_set_phase_status({phase: "context", ...})` call). Pass both to ALL agents as context — read them back with `mcp__claude-pipeline__pipeline_state_get` whenever the orchestrator needs them.
...
Record `complexity` via `mcp__claude-pipeline__pipeline_set_phase_status` (or any MCP write call that accepts the field). The MCP rebuilds `pipeline-state-summary.md` automatically — do NOT hand-edit either file.
...
Record the decision:
```
mcp__claude-pipeline__pipeline_set_gate({
  project_dir,
  gate: "gate0",
  decision: "<approved|rejected>",
  feedback: "<user's verbatim response or summary>"
})
```
...
### ⛔ HUMAN GATE 2
Before presenting, run `mcp__claude-pipeline__pipeline_validate({project_dir})` and surface any violations alongside the summary.
...
- Any MCP invariant violations from `pipeline_validate`
...
## Global Rules
1. After **every agent completion**, call the appropriate MCP recorder:
   - **Reviewer/validator agent** (logic, challenger, style, security, performance, acceptance, plan-conformance, plan-grounding-check, context-doc-verifier, ui-consistency, api-contract, playwright, test):
     ```
     mcp__claude-pipeline__pipeline_record_agent_run({
       project_dir,
       phase: "<context|planning|test_first|implementation|validation>",
       agent: "<agent name>",
       agent_output: "<entire agent text output, including the fenced ```json header>"
     })
     ```
     The MCP parses the JSON header, validates it against `reviewer-output.schema.json` / `validator-output.schema.json`, streams each `findings[]` entry into `.claude/findings.jsonl` (each validated against `finding.schema.json`), appends to `reviewer_verdicts[]`, increments `agents_count`, and rebuilds `pipeline-state-summary.md`.
   - **Non-reviewer agent** (Planner, Implementer, Architect, Code Analyzer, Dependency Auditor, Research, Migration):
     ```
     mcp__claude-pipeline__pipeline_record_nonreview_agent({
       project_dir,
       phase: "<phase>",
       agent: "<agent name>",
       output_file: "<optional path to the artifact, e.g. .claude/plan.md>",
       iterations: <optional integer>
     })
     ```
   When transitioning a phase's lifecycle, also call `mcp__claude-pipeline__pipeline_set_phase_status({project_dir, phase, status, skipped_reason?})`.

   **Never use `Write` or `Edit` on `.claude/pipeline-state.json`, `.claude/pipeline-state-summary.md`, or `.claude/findings.jsonl`.** The MCP layer is the only writer. Hand-edits bypass schema validation and invariant checks and break `/done`.
...
11. **Exact counts in pipeline-state.json:** `agents_count` and iteration numbers are integers, never approximations or strings with `~N`. The MCP layer enforces this — each `pipeline_record_agent_run` / `pipeline_record_nonreview_agent` call atomically increments `agents_count` by exactly 1, so the value always matches the actual number of spawned agents recorded.
...
15. **Past Misses injection (cached per pipeline run, JSONL-aware):** ONCE at pipeline start (right after STEP 1), for each reviewer that consumes past misses, call:
    ```
    mcp__claude-pipeline__pipeline_get_past_misses({
      agent: "<logic-reviewer|challenger-reviewer|style-reviewer|security|performance>",
      limit: 10,
      human_confirmed_only: true
    })
    ```
    The MCP reads `~/.claude/metrics/agent-feedback.jsonl`, filters by `agent` AND `human_confirmed=true`, and returns the last N entries already in the canonical line format:
    ```
    - [YYYY-MM-DD] [category=<cat>] <pattern_to_look_for> — example: <file:line> — severity: <high|medium|low>
    ```
    Write each result to its per-agent file (`.claude/past-misses-<agent>.md`). If the MCP returns zero entries, write `(no past-miss data)`. Reviewer receives the **path** and Reads it.

    **Diff-aware filtering (advanced, optional):** if `.claude/diff.txt` is available at injection time and the diff shape suggests certain categories (e.g. diff touches `*.sql` → prefer entries with `category` in {n-plus-one, missing-index, full-table-scan, ...}), pass `category_hint` to `pipeline_get_past_misses` to bias the ranking. Default: chronological last-10.
...
22. **Pipeline-state JSON shape is fixed and enforced by the MCP server.** Findings stream goes to `.claude/findings.jsonl` (append-only, one JSON object per line, MCP-managed). Per-agent reviewer outputs go to `.claude/reviews/<agent>-<iter>.md` (full markdown narrative + json header) for audit; the JSON state only references them via `reviewer_verdicts[]`. File-pointer policy: reviewers Read these review files themselves — never inline them into reviewer prompts.
23. **Structured findings stream:** every reviewer/validator's `findings[]` array is appended (as individual JSON objects) to `.claude/findings.jsonl` **via `mcp__claude-pipeline__pipeline_record_agent_run`**, which validates each finding against `finding.schema.json` and rejects invalid entries. This is the source `/learn` (Phase G) reads to cluster categories, detect drift, and propose vocab updates. Never `Write` or `Edit` `findings.jsonl` directly.
24. **Mechanical metrics extraction at /done:** The metrics line in `~/.claude/metrics/pipeline.jsonl` is built **inside the MCP** by `mcp__claude-pipeline__pipeline_finish` — pure JSON-to-JSON transform from `.claude/pipeline-state.json`, no LLM call, refuses on any invariant violation. Only the commit-message generation in `/done` involves the LLM.
25. **TDD enforcement (when `tests_mode=tdd`):** all state mutations below go through `mcp__claude-pipeline__pipeline_record_agent_run` / `pipeline_record_nonreview_agent` / `pipeline_set_phase_status` — never via direct file writes.
    ...
    - **b. Count test specs in plan.** Scan plan for headings `### Test T<N>:` and sub-headings `#### Case T<N>.<x>:`. Pass the count via `pipeline_record_nonreview_agent({phase: "planning", ...})` extras so the MCP records `phases.test_first.test_spec_count_in_plan = <count of cases>`.
    - **c. After Test Agent (RED) completes:** the MCP parses the JSON header (via `pipeline_record_agent_run`) and the orchestrator reads back `details.totals.failing_expected` from `pipeline_state_get`. If `< test_spec_count_in_plan`, that's a **count mismatch** — Test Agent didn't write all declared cases. Treat as `verdict: ERROR`, re-spawn Test Agent ONCE with the missing-cases list. If still mismatched after retry → STOP, escalate to human.
    - **d. Record sacred test hashes.** After RED is verified, walk every file in `phases.test_first.test_files_written` and compute its sha256. Pass the `{path: hash}` map to `pipeline_set_phase_status({phase: "test_first", status: "completed", test_files_hashes_post_red: {...}})` so the MCP stores them.
    - **e. Write `.claude/test-files-must-stay-green.json`** — the explicit list of test file paths. Implementer reads this as input. (This is a working file, not a state mutation — `Write` is fine.)
    - **f. After STEP 6 (GREEN):** before plan-conformance, recompute hashes for every file in the sacred list. Read stored hashes via `pipeline_state_get` and diff. Any mismatched path → pass to `pipeline_set_phase_status({phase: "implementation", test_files_modified_by_implementer: [...]})`. plan-conformance treats each as blocking finding `category: "test-file-modified-by-implementer"`. The human at Gate 2 must explicitly approve via `pipeline_set_gate({gate: "gate2", decision: "approved", feedback: "approves sacred-test modification: <path>"})` OR the Implementer reverts the change.
...
26. **JSON output validation (brittleness mitigation).** Every reviewer/validator agent emits a fenced ```json block at the top. `mcp__claude-pipeline__pipeline_record_agent_run` validates this block against `templates/schemas/{reviewer,validator}-output.schema.json` and returns an error on failure. On error:
    ...
    - **Persistent failure.** Call `pipeline_record_agent_run` once more with a synthetic minimal-valid output that records the violation, OR escalate to human and let the MCP write a `pipeline_violation` entry. The MCP appends the failed-output record to `findings.jsonl` only after schema validation succeeds; never bypass with a direct file write. Surface to human at next gate. Do NOT silently parse markdown narrative as a fallback.
...
29. **STEP 5 skip logging.** If STEP 5 is skipped (because `tests_mode=regression-only`), call:
    ```
    mcp__claude-pipeline__pipeline_set_phase_status({
      project_dir,
      phase: "test_first",
      status: "skipped",
      skipped_reason: "<one of: regression-only | user-override-no-tests | no-test-framework-tdd-blocked>"
    })
    ```
    The MCP rejects `status: "skipped"` for `test_first` without `skipped_reason` (invariant INV_003). Never silently skip without recording the reason.
