...
| `INV_013` | `acceptance.verdict ∈ {PASS, PASS_WITH_WARNINGS}` ⇒ no open `severity: blocking` findings emitted by impl-phase reviewers at the latest impl iteration. Fires from `pipeline_record_agent_run` post-record AND `pipeline_finish` pre-emit. Real-task driver (Q68, frontend-core 2026-05-19): style-reviewer at impl iter=2 emitted 3 blocking prettier violations on new files; acceptance subsequently returned PASS because its `pnpm test/lint/build` call ran against project scope where the same files were excluded. Without INV_013 the task would have shipped with blockers open. |
