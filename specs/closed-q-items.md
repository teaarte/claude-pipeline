...
- **56 validation-driven Q-items closed** across 7 bundles (v2.1-hotfix, v2.1-polish-bundle, v2.2-clear-bundle, v2.2a-review-completeness, v2.2.5-bundle-foundation + followups, v2.2.6-stack-classifier, v2.2.7-classifier-portability).
...
## v2.2.7-classifier-portability (branch `v2.2.7-classifier-portability`, 2026-05-19)

| Q | Severity | Commit | What |
|---|---|---|---|
| Q74 | 🔴 CRITICAL | `777141d` | **Gate-2 reject FSM-control-flow fix.** `UserAnswer.reject_intent: "revise" \| "abandon"`. gate-2 reject-revise walks step_index back to impl entry; reject-abandon finalizes verdict=rejected. FINALIZE throws `INV_inconsistent-finalize` when verdict=null (no more silent default to "accepted"). |
| Q68 | 🔴 HIGH | `e4ab54d` | **INV_013 acceptance gate on impl-blockers.** `acceptance.verdict=PASS` with open `severity: blocking` findings from impl-phase reviewers at the latest iteration → invariant fires at `pipeline_record_agent_run` post-record AND `pipeline_finish` pre-emit. acceptance.md prompt + `mcp/src/lib/invariants.ts`. |
| Q73 | 🟢 LOW | `f8092bb` | **`summary_line` schema cap raised 100 → 150 chars.** Eliminates the lenient-parse retry noise on legitimate single-line summaries on complex tasks. reviewer-output + validator-output schemas. |
| Q66 | 🟢 LOW | `68c8b56` | **Configurable project subdir via `CLAUDE_PIPELINE_PROJECT_SUBDIR` env var.** `mcp/src/lib/paths.ts` reads the env var with default `.claude` (existing CC projects unchanged). Headless / daemon / non-CC users can override. Documented in `mcp/README.md`. |
| Q72 | 🟡 MEDIUM | `620fcb3` | **`owner_id` env-var docs + audit warning on null.** `mcp/README.md` documents the required CC mcpServers env config; `pipelineRunTask` emits `audit error_class: "owner-id-unset"` when ownerId resolves null so the gap is visible in metrics. |
| (D3) | — | `93df722` | **Implementer tech-debt auto-capture from prose.** New `extract-tech-debt-from-prose` after-agent-result hook with implementer filter. Greps for `pre-existing` / `out-of-scope` / `not a regression` / `noticed` / `TODO:` / `FIXME:` paragraph signals; appends matches to `.claude/issues-found.md` under `<!-- auto-captured hash=... -->` markers (idempotent). |
| Q71 | 🟡 MEDIUM | `a277d53` | **Gate-0 / gate-1 prompts truncate the task body.** `shortTask(state)` helper prefers `state.task_short` (populated by D1's classifier), falls back to first-line + 80-char ellipsis. Real-task evidence: 10-KB task body forced human to scroll past their own input. Snapshot test asserts gate-1 message stays under 500 chars. |
| Q-classifier-auto-spawn (Item 1) | — | `1f03388` | **Classifier-agent auto-spawn in context phase.** New `CLASSIFY_AGENT` step spawns the classifier; new `extract-classifier-output` after-agent-result hook parses the JSON against `classifier-output.schema.json` and populates `state.decisions.{task_short, refs_to_load, security_needed, antipattern_rules_applicable, stack, change_kind}`. Failure mode: keep defaults, audit `llm-classification-needed`, continue. `mockClassifierSpawnProvider()` test helper for FSM unit tests. |
| Q-change-kind (Item 2) | — | `9a53d54` | **Reviewer selectivity by `change_kind`.** `AgentPlugin` gains `relevant_for_change_kinds?: string[]`. Style + performance skip type-only / docs-only / config-only diffs (real-task evidence: 0 findings on type-only TS diff = ~10K tokens of waste). Conservative defaults: null/undefined `change_kind` → spawn all eligible reviewers. Each skip emits audit `reviewer-skipped-change-kind`. |
| Q65 | 🟡 MEDIUM | `d3f36b6` | **Generic shuttle response shape.** `ClaudeCodeTaskSpec` → `SpawnRequest { runner_hint, description, prompt, model?, extras }`. `DriverResponse.claude_code_task` → `spawn_request`. CC-specific `subagent_type` moves to `extras`. Audit log gains `runner_hint`. Skill markdown (`commands/task.md`) branches on `runner_hint`. Zero matches of `claude_code_task` in `mcp/src/` post-rename. |
| Q67 | 🟡 MEDIUM | `6afa4a6` | **Parallelize planning-phase reviewers.** MEDIUM + COMPLEX flows merge `plan-grounding` into `plan-review`'s `spawnAgentsParallel` fan-out over `[plan-grounding-check, logic-reviewer]`. One shuttle round-trip instead of two; ~30-60s saved per medium/complex task. SIMPLE flow keeps standalone `plan-grounding`. |
| Q69 | 🟡 MEDIUM | `c34640a` | **Auto-derived gate-1 feedback.** `gate-1.message()` aggregates latest planning-phase reviewer findings into a pre-filled "Suggested revision" block sorted by severity → agent → id (capped at 8). `UserAnswer.decision` gains `"auto-apply"` — continue-task substitutes the stashed revision block as reject feedback. Skill markdown adds gate-1-specific parse (1/a/auto-apply, 2/accept-anyway, 3/edit, 4/reject). |
| Q70 | 🟢 LOW | `20e9648` | **Opt-in auto-replan loop at gate-1.** `pipeline.config.json` `auto_replan_on_blocking_max: 0 \| 1 \| 2` (default 0). When set + blocking planning findings exist + count < cap → gate-1's pre-ask hook walks step_index back to PLAN; no human pause. Audit row `error_class: "auto-replan"` per iteration. Only BLOCKING findings burn the cap. |
