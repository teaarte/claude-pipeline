...
| Q9 | 🟡 MEDIUM | open — **root cause identified 2026-05-14** | **Code review under-spawning — WIRING BUG.** Investigation on `t-...-addauthtokendecodert` (5th recurrence, security-touched task) revealed: `driver-state.decisions` for this task contains only `complexity`, `tests_mode`, `refs_to_load`. **`security_needed`, `ui_touched`, `api_touched` are NEVER computed.** Their DecisionPlugins are registered in `loaders/builtins.ts` but **no flow step invokes them** before the `review` step. Therefore `applies_to` predicates on `security_needed`/`ui_touched`/`api_touched`-gated reviewers always evaluate against undefined → effectively false → reviewers never spawn. **Confirmed eliminated hypotheses:** (a) "predicates too aggressive" — predicates never even run; (b) "spawn step bug" — we don't reach spawn for gated reviewers; (c) "Gate 1 revision resets decisions" — this run had revision (REQUEST_CHANGES → APPROVE) and `security_needed` still absent. **Concrete fix:** add a "compute-review-decisions" step (or extend existing classify step) in `builtin/flows/{medium,complex}.ts` that calls `state.invokeDecision()` for `security-needed`, `ui-touched`, `api-touched` BEFORE the `review` step. Validate via `driver-state.decisions` containing all three keys after planning phase closes. ~1-2h fix (was estimated ~2-3h; narrowing the root cause shrank it). |
...
| Q41 | 🟡 MEDIUM | open | **`refs-to-load` decision is stack-blind** — uses task-text regex only, picks backend refs (db-postgres, redis, concurrency) for frontend monorepo tasks. **Architectural fix:** refs become self-describing (YAML frontmatter); decision becomes async + LLM-driven. Bundle into v2.2a alongside Q9 + Q27 + Q30. ~1d. |
| Q42 | 🟡 MEDIUM | open | **`task_id` slug collision** — two different tasks (Step 3 Identity, Step 4 _demo-contract) on s3-panel produced same `task_id: t-2026-05-14-contextreadfirstinth` because both start with `## Context (read first, in this order)` preamble. Cross-task aggregation by task_id silently joins unrelated tasks. Fix: hash-suffix when collision detected. ~1h. |
| Q43 | 🟢 LOW | open | **`impl_iters` overcounts on cross-phase same-agent.** Iteration counter is global-per-agent (logic-reviewer iter=2 = "2nd time this agent ran overall", not "2nd iter of implementation review"). Q22 derive `max(iteration) WHERE phase=X` reads wrong value. Fix: change derive to `count(verdicts WHERE phase=X)` in `tools/finish.ts`. Bundle with Q9 in v2.2a. ~30min. |
