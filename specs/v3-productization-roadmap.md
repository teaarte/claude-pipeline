...
| Q27 | 🟡 MEDIUM | ✓ fixed (v2.2a-review-completeness) | **Pre-review infrastructure files missing.** All four expected files now emit at `before-step` on the `review` step. **Fixed: v2.2a-review-completeness Q27** — four hooks in `mcp/src/driver/builtin/hooks/index.ts`: `git-diff-snapshot` writes `.claude/diff.txt` via `git diff` (stub on non-repo); `load-past-misses` writes one `.claude/past-misses-<agent>.md` per reviewer (logic / challenger / style / security / performance) via `pipelineGetPastMisses`; `anti-pattern-grep` parses CLAUDE.md "What NOT to do" rules and surfaces token-overlap candidates against the staged diff; `caller-context-expand` extracts function names from the diff and uses `git grep` to find callers (capped at 30 sites), MEDIUM/COMPLEX only. Each hook degrades to an explicit stub when its source is empty. 12 new tests covering happy paths, missing sources, simple-skip, and internal helpers. |
...
| Q30 | 🟡 MEDIUM | ✓ fixed (v2.2a-review-completeness) | **`refs_loaded` always empty.** Refs-to-load decision ran in driver-state but result never reached canonical pipeline-state. **Fixed: v2.2a-review-completeness Q30** — `set-phase-status.ts` now reads `driver-state.json` when closing the planning phase and copies `decisions.refs_to_load` → `state.refs_loaded` and `decisions.refs_dropped_due_to_cap` → `state.refs_dropped_due_to_cap`. Missing driver-state degrades silently (initial template `[]` preserved). 2 new tests in `set-phase-status.test.ts`. |
...
| Q41 | 🟡 MEDIUM | 🟡 **partial** (v2.2a — regex fallback live in prod; LLM path awaits v2.3 non-shuttle SpawnProvider) | **`refs-to-load` decision is stack-blind** — uses task-text regex only, picks backend refs for frontend tasks. **Partial fix in v2.2a-review-completeness Q41:** refs are now self-describing via YAML frontmatter on all 25 `agents/references/*.md` files (tags, stack_signals, summary, when_to_load, agent_hints). `DecisionPlugin.decide(state, ctx?)` contract evolved with optional `DecisionContext{active_agents, spawn_provider}` — existing decisions unchanged. `SpawnProviderPlugin.query?()` added as optional one-shot LLM classification method. `refs-to-load.ts` rewritten: LLM-driven when `ctx.spawn_provider.query` is present, regex fallback otherwise (≥ pre-Q41 quality always). Frontmatter parser in `mcp/src/lib/parse-frontmatter.ts` (hand-rolled, no dep). `classify` step passes `{active_agents: flow.steps, spawn_provider}` through. 19 new tests (parser, refs-metadata loader scanning all 25 real ref files, refs-to-load LLM path / fallback / cap / hallucination filter / cached / malformed-output, prompt builder, parsePickedRefs). **In production today:** shuttle SpawnProvider leaves `query?()` undefined (shuttle pattern can't issue synchronous out-of-band LLM calls), so the regex fallback path is what real `/task` runs use. Stack-blind selection behavior is therefore unchanged in prod despite the contract + frontmatter being in place. Full LLM activation requires v2.3 daemon's non-shuttle SpawnProvider (Anthropic SDK direct) — at which point refs-to-load automatically picks up the LLM path with zero code changes. |
...
- **Closed in v2.2a-review-completeness (5 + 1 partial):** Q9, Q27, Q30, Q42, Q43 fully shipped. **Q41 partial** — contract + frontmatter (25 refs) + tests in place, but LLM classification path inactive in prod because shuttle SpawnProvider doesn't implement `query?()` (shuttle pattern incompatible with synchronous out-of-band LLM calls). Regex fallback active = pre-Q41 quality preserved, not improved. Full activation pending v2.3 daemon's non-shuttle SpawnProvider (Anthropic SDK direct). Real-task validation pending post-merge for the 5 fully-shipped items.
...
Total validation-driven backlog: 28 items surfaced across 5 real-task runs (auth-decoder added Q43, narrowed Q9 root cause). 27 closed, 0 open from validation-driven set; 2 deferred (Q38, Q40); separate code-polish bundle Q1-Q6 untouched.
