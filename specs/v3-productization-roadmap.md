...
**v2.1-polish-bundle status:** 10 of 17 open items closed (Q8, Q11, Q14, Q17-Q24). **Still open: 7 items** — Q9 (under-spawning, deferred — needs auth/perf real-task), Q10 (cosmetic — `current_step` stale), Q25 (onboarding friction, docs-only), Q26 (Q17 partial — stack values wrong), Q27 (pre-review infra missing — top v2.2 priority with Q9), Q28 (Q21 partial — schema_version per-finding), Q29 (logic-reviewer vocab gap). Q1-Q6 code-quality items deferred to separate `v2.2-code-polish` bundle.

**v2.2 next-bundle candidate (surfaced by post-bundle real-runs):** Q9 + Q27 are tightly coupled — fixing review fan-out without fixing review inputs is incomplete. Bundle these two together as v2.2-review-completeness (~5-7d). Q26 is independent and small (~1-2h). Q28 + Q29 are mechanical prompt/vocab edits (~1h combined).
